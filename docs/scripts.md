...
3. **Critical binaries in `PATH`** — confirms core tools (`stow`, `hyprctl`, `waybar`, `rofi`, `dunst`, etc. — see `DEFAULT_CRITICAL_BINS` in the script) are reachable. Override the list by passing binary names as arguments.
